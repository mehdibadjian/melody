---
name: loop-resolve
description: 'Interactive escalation-resolution workflow for the myloop orchestrator. A myloop run paused on a CRITICAL escalation (a contradiction or gap a dev/review session could not safely resolve alone); you and the human disambiguate the frozen spec so the story can be re-driven. Invoked as /loop-resolve <story-key>. Unlike the automated dev/review sessions this session is interactive — a human is present and you SHOULD ask.'
---

# myloop Escalation Resolution

A `myloop` run drove a story through dev → review, a session raised a
...
recommend — but the human makes the call. (`$MYLOOP_MODE` is intentionally
...
- `$MYLOOP_RUN_DIR` — the paused run's directory.
- `$MYLOOP_STORY_KEY` — the escalated story key (also your invocation argument).
- `$MYLOOP_RESOLVE_CONTEXT` — path to a `context.json` written for you.

**Read `$MYLOOP_RESOLVE_CONTEXT` FIRST.** Its schema:

```json
{
  "story_key": "6-4-cli-list-command",
  "run_id": "20260613-111429-6a14",
  "spec_file": "/abs/path/to/docs/implementation-artifacts/spec-<story>.md",
  "baseline_commit": "<sha>",
...
4. **Update the frozen spec** to encode the decision unambiguously: amend the
   ...
   smallest change that removes the ambiguity. You MAY use the `spec` or
   `correct-course` skills if a larger spec change is warranted.
