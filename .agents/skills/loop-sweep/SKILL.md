---
name: loop-sweep
description: 'Triage the deferred-work ledger for the myloop orchestrator: verify every open entry against the actual codebase and return a machine-readable partition (bundles, already-resolved, blocked, skip, human decisions). Also migrates legacy pre-DW-format ledgers when invoked with --migrate. Automation-only — invoked by myloop sweep runs, not by humans.'
---
...
`myloop` sweep session. You never edit the ledger, never edit code, never
...
Run: `echo "${MYLOOP_MODE:-}"`

If the output is not `1`, state that this skill only runs inside a myloop
...
Read `{project-root}/.myloop-config/core/config.yaml` to resolve `implementation_artifacts`,
