...
inner dev path the build-auto session appends its own flat entries (review
...
`myloop sweep` migration rewrites freeform pre-DW-format content from older
...
`myloop sweep` runs (the orchestrator and its bundle dev sessions) add two
...
- **Never a gate.** An id that matches no entry, an entry whose `status:` reads
  ...
  none can fail the story. `myloop validate` reports the same mismatches as
