...
You are running unattended inside a `myloop` sweep session. No human is
...
- `$MYLOOP_RUN_DIR` and `$MYLOOP_TASK_ID` are set in your environment.
- Your **result file** is `$MYLOOP_RUN_DIR/tasks/$MYLOOP_TASK_ID/result.json`.
...
- Your **escalation file** is `$MYLOOP_RUN_DIR/tasks/$MYLOOP_TASK_ID/escalation.json`.
